...
template <typename T>
class GroupConv2d : public delegator::GroupConv2d {
 public:
  explicit GroupConv2d(const delegator::GroupConv2dParam &param)
      : delegator::GroupConv2d(param) {}
  ~GroupConv2d() {}
...
template <typename T>
MaceStatus GroupConv2d<T>::Compute(const OpContext *context,
                                   const Tensor *input,
                                   const Tensor *filter,
                                   Tensor *output) {
  MACE_UNUSED(context);
  ...
  if (paddings_.empty()) {
    CalcNCHWPaddingAndOutputSize(
        input->shape().data(), filter->shape().data(), dilations_.data(),
        strides_.data(), padding_type_, out_shape.data(), paddings.data());
  } else {
    paddings = paddings_;
    CalcNCHWOutputSize(input->shape().data(), filter->shape().data(),
                       paddings_.data(), dilations_.data(), strides_.data(),
                       RoundType::FLOOR, out_shape.data());
  }
  ...
  for (index_t b = 0; b < in_shape[0]; b++) {
    for (index_t m = 0; m < filter_shape[0]; ++m) {
      ...
      T *out_ptr_base = output_data + b * out_batch_size + m * out_image_size;
          ...
          for (index_t c = 0; c < in_channels; ++c) {
            ...
            for (index_t kh = 0; kh < filter_shape[2]; ++kh) {
              for (index_t kw = 0; kw < filter_shape[3]; ++kw) {
                const index_t ih =
                    -pad_top + h * strides_[0] + kh * dilations_[0];
                const index_t iw =
                    -pad_left + w * strides_[1] + kw * dilations_[1];
...
void RegisterGroupConv2dDelegator(OpDelegatorRegistry *registry) {
  MACE_REGISTER_DELEGATOR(registry, GroupConv2d<float>,
                          delegator::GroupConv2dParam,
                          MACE_DELEGATOR_KEY(GroupConv2d, RuntimeType::RT_CPU,
                                             float, ImplType::REF));

  // FP16 and BF16 are not supported for GroupConv2d CPU
  //   MACE_REGISTER_BF16_DELEGATOR(
  //       registry, GroupConv2d<BFloat16>, delegator::GroupConv2dParam,
  //       MACE_DELEGATOR_KEY(GroupConv2d, RuntimeType::RT_CPU, BFloat16,
  //                          ImplType::REF));

  //   MACE_REGISTER_FP16_DELEGATOR(
  //       registry, GroupConv2d<float16_t>, delegator::GroupConv2dParam,
  //       MACE_DELEGATOR_KEY(GroupConv2d, RuntimeType::RT_CPU, float16_t,
  //                          ImplType::REF));
}
