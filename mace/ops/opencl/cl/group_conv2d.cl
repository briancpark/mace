...
__kernel void group_conv2d(
    OUT_OF_RANGE_PARAMS GLOBAL_WORK_GROUP_SIZE_DIM3 __read_only image2d_t
        input,                    /* [c%4 * w * c/4, h * b] */
  ...
  int in_height_gap = select(0, (-height_start + dilation_h - 1) / dilation_h,
                             height_start < 0);
  int in_height_start = mad24(in_height_gap, dilation_h, height_start);
  int in_height_end =
      min(mad24(filter_height, dilation_h, height_start), in_height);
...
#if defined(USE_RELU) || defined(USE_LEAKYRELU) || defined(USE_RELUX) || \
    defined(USE_TANH) || defined(USE_SIGMOID) || defined(USE_ELU)
