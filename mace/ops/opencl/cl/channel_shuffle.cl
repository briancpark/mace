...
__kernel void channel_shuffle(OUT_OF_RANGE_PARAMS GLOBAL_WORK_GROUP_SIZE_DIM3
  ...
  const int group_blks = groups >> 1;  // Use 2 groups
  const int groups_blks_width = mul24(group_blks, width);
  const int channels_per_group_blks = channels_per_group >> 2;
  const int channels_per_group_blks_width =
      mul24(channels_per_group_blks, width);

  DATA_TYPE4 in_chan_data0, in_chan_data1;
  DATA_TYPE4 out_chan_data0, out_chan_data1;

  int in_x = mad24(group_chan_blk_idx, width, width_idx);
  for (short g_blk = 0; g_blk < group_blks; ++g_blk) {
    // fetch 2 groups, for each group fetch 4 channels
    ...
    out_chan_data0 = (DATA_TYPE4)(in_chan_data0.x, in_chan_data1.x,
                                  in_chan_data0.y, in_chan_data1.y);
    out_chan_data1 = (DATA_TYPE4)(in_chan_data0.z, in_chan_data1.z,
                                  in_chan_data0.w, in_chan_data1.w);

    int out_x =
        mad24(mad24(group_chan_blk_idx, groups, g_blk), width, width_idx);
