...
//         std::max<uint32_t>(executor->device_compute_units() / 2, 1);
//     const uint32_t base = std::max<uint32_t>(
//         std::min<uint32_t>(cache_size / kBaseGPUMemCacheSize, 4), 1);
//     lws[1] = std::min<uint32_t>(gws[1], kwg_size);
//     lws[0] =
//         std::min<uint32_t>(std::min<uint32_t>(gws[0], base), kwg_size /
//         lws[1]);
...
//     }
//     lws[2] =
//         std::max<uint32_t>(std::min<uint32_t>(lws[2], kwg_size / lws_size),
//         1);
//   }
//   return lws;
// }
...
MaceStatus GroupConv2dK3x3(OpContext *context,
                           cl::Kernel *kernel,
                           const Tensor *input,
                           const Tensor *filter,
                           const Tensor *bias,
