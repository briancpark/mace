...
#include "mace/ops/arm/base/group_conv2d.h"
...
void GroupConv2dBase::CalOutputShapeAndInputPadSize(
    const std::vector<index_t> &input_shape,
    const std::vector<index_t> &filter_shape,
    std::vector<index_t> *output_shape,
    std::vector<int> *in_pad_size) {
  if (paddings_.empty()) {
    CalcNCHWPaddingAndOutputSize(input_shape.data(), filter_shape.data(),
                                 dilations_.data(), strides_.data(),
                                 padding_type_, output_shape->data(),
                                 in_pad_size->data());
  } else {
    *in_pad_size = paddings_;
    CalcNCHWOutputSize(input_shape.data(), filter_shape.data(),
                       paddings_.data(), dilations_.data(), strides_.data(),
                       RoundType::FLOOR, output_shape->data());
  }
}

void GroupConv2dBase::CalOutputBoundaryWithoutUsingInputPad(
...
void GroupConv2dBase::CalOutputShapeAndPadSize(
    const Tensor *input,
    const Tensor *filter,
  ...
  CalOutputShapeAndInputPadSize(input->shape(), filter->shape(), output_shape,
                                &paddings);

  const index_t out_height = (*output_shape)[2];
  const index_t out_width = (*output_shape)[3];
  const index_t padded_out_height =
      RoundUp<index_t>(out_height, out_tile_height);
...
MaceStatus GroupConv2dBase::ResizeOutAndPadInOut(
  ...
  std::vector<int> in_pad_size;
  std::vector<int> out_pad_size;
  CalOutputShapeAndPadSize(input, filter, out_tile_height, out_tile_width,
  ...
  if (is_in_padded) {
    std::vector<index_t> padded_in_shape = {batch, in_channels,
                                            padded_in_height, padded_in_width};
    std::unique_ptr<Tensor> padded_in = make_unique<Tensor>(
        runtime, input->dtype(), MemoryType::CPU_BUFFER, padded_in_shape);
    runtime->AllocateBufferForTensor(padded_in.get(), RENT_SCRATCH);
    MACE_CHECK(padded_in->data<float>() != nullptr);
  ...
  if (is_out_padded) {
    ...
    std::unique_ptr<Tensor> padded_out = make_unique<Tensor>(
        runtime, output->dtype(), MemoryType::CPU_BUFFER, padded_out_shape);
    runtime->AllocateBufferForTensor(padded_out.get(), RENT_SCRATCH);
    *padded_output = std::move(padded_out);
  }
  return MaceStatus::MACE_SUCCESS;
}

void GroupConv2dBase::PadInput(const Tensor &src,
                               const int pad_top,
                               const int pad_left,
                               Tensor *dst) {
  if (dst == &src) return;
  ...
  MACE_CHECK(padded_in_data != nullptr);
  MACE_CHECK(in_data != nullptr);
  ...
  for (index_t b = 0; b < batch; ++b) {
    for (index_t c = 0; c < channels; ++c) {
      ...
      for (index_t h = 0; h < height; ++h) {
        memset(padded_in_base, 0, type_size_ * pad_left);
        memcpy(padded_in_base + pad_left * type_size_, in_base,
               type_size_ * width);
        memset(padded_in_base + (pad_left + width) * type_size_, 0,
...
void GroupConv2dBase::UnPadOutput(const Tensor &src, Tensor *dst) {
  if (dst == &src) return;
