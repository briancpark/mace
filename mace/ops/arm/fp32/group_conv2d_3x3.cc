...
MaceStatus GroupConv2dK3x3S1<float>::DoCompute(const GroupConvComputeParam &p,
  ...
  VLOG(3) << output_data;
  VLOG(3) << p.batch;
  VLOG(3) << "NEED TO IMPLEMENT GroupConv2dK3x3S1<float>::DoCompute";
  ...
  //                 vf_outch_height float32x4_t vf00, vf01, vf02; float32x4_t
  //                 vf10, vf11, vf12; vf00 = vld1q_f32(filter_ptr0); vf01 =
  //                 vld1q_f32(filter_ptr0 + 3); vf02 = vld1q_f32(filter_ptr0 +
  //                 6);
  ...
  //                     // output (4 outch x 2 height x 4 width):
  //                     vo_outch_height float32x4_t vo00, vo01; float32x4_t
  //                     vo10, vo11;
  ...
  //                     // outch 0, height 0
  //                     vo00 =
  //                         vfmaq_laneq_f32(vo00, vi00, vf00, 0);  // reg
  //                         count: 18
  ...
  //                 float *out_ptr0_base =
  //                     output_data + b * p.out_batch_size + mm *
  //                     p.out_image_size;
  //                 for (index_t c = 0; c < p.in_channels; ++c) {
  //                   const float *in_ptr0 =
  //                       input_data + b * p.in_batch_size + c *
  //                       p.in_image_size;
  //                   const float *in_ptr1 = input_data + b * p.in_batch_size +
  //                                          c * p.in_image_size + 1 *
  //                                          p.in_width;
  ...
  //                                          p.in_width;
  //                   const float *filter_ptr0 =
  //                       filter_data + mm * p.in_channels * 9 + c * 9;

  //                   float *out_ptr0 = out_ptr0_base;

  //                   // load filter (1 outch x 3 height x 3 width):
  //                   vf_outch_height float32x4_t vf00, vf01, vf02; vf00 =
  //                   vld1q_f32(filter_ptr0); vf01 = vld1q_f32(filter_ptr0 +
  //                   3); vf02 = vld1q_f32(filter_ptr0 + 5);
  ...
  //                       vo_outch_height float32x4_t vo00, vo01;
...
MaceStatus GroupConv2dK3x3S2<float>::DoCompute(const GroupConvComputeParam &p,
  ...
  VLOG(3) << output_data;
  VLOG(3) << p.batch;
  VLOG(3) << "NEED TO IMPLEMENT GroupConv2dK3x3S2<float>::DoCompute";
  //   p.thread_pool.Compute2D(
  //       [=](index_t start0, index_t end0, index_t step0, index_t start1,
  //           index_t end1, index_t step1) {
  //         for (index_t b = start0; b < end0; b += step0) {
